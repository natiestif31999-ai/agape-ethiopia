...
function Header({ eyebrow, title, body }: { eyebrow: string; title: string; body?: string }) {
  return <><Text style={styles.eyebrow}>{eyebrow}</Text><Text style={styles.heading}>{title}</Text>{body ? <Text style={styles.subtitle}>{body}</Text> : null}</>;
}
...
function SyncBanner({ online, pending, onPress }: { online: boolean; pending: number; onPress: () => void }) {
  const { t } = useTranslation();
  const label = !online ? t("offline") : pending ? `${pending} ${t("waiting")}` : t("synced");
  return <Pressable accessibilityRole="button" onPress={onPress} style={[styles.syncBanner, !online && styles.offlineBanner]}><View style={[styles.syncDot, !online && styles.offlineDot]} /><Text style={styles.syncText}>{label}</Text><Text style={styles.syncArrow}>›</Text></Pressable>;
}

function HomeScreen({ navigation }: NativeStackScreenProps<RootStackParamList, "Home">) {
  const { t, locale, setLocale } = useTranslation();
  const { online, pending, failed } = useQueueStatus();
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}><Image source={logo} style={styles.logo} resizeMode="contain" /><Text style={styles.brand}>{t("brand")}</Text><Text style={styles.welcome}>{t("welcome")}</Text><Text style={styles.subtitle}>{t("homeBody")}</Text><View style={styles.languageRow}><Text style={styles.languageLabel}>{t("language")}</Text>{(["en", "am", "om", "ti"] as const).map((item) => <Pressable key={item} onPress={() => setLocale(item)} style={[styles.languageChip, locale === item && styles.chipActive]}><Text style={locale === item ? styles.chipTextActive : styles.chipText}>{item.toUpperCase()}</Text></Pressable>)}</View>{failed > 0 ? <Pressable onPress={() => navigation.navigate("Offline")} style={[styles.syncBanner, styles.failedBanner]}><View style={[styles.syncDot, styles.failedDot]} /><Text style={styles.syncText}>{failed} {t("failedSummary")}</Text><Text style={styles.syncArrow}>›</Text></Pressable> : <SyncBanner online={online} pending={pending} onPress={() => navigation.navigate("Offline")} />}<View style={styles.actions}><Action icon="+" title={t("newBeneficiary")} body={t("newBeneficiaryBody")} onPress={() => navigation.navigate("Register")} /><Action icon="+ +" title={t("multiple")} body={t("multipleBody")} onPress={() => navigation.navigate("Register", { multiple: true })} /><Action icon="↔" title={t("partnership")} body={t("partnershipBody")} onPress={() => navigation.navigate("Partnership")} /></View><Button label={t("offlineRecords")} secondary onPress={() => navigation.navigate("Offline")} /><Text style={styles.footer}>{t("phase")}</Text></ScrollView></SafeAreaView>;
}
